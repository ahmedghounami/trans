...
export default function ProfileHeader({user, games }: { user: any [], games: any[] }) {
    ...
    const goldearned = games.reduce((total, game) => {
        return total + (game.player1_id === user.id ? game.player1_gold_earned : game.player2_gold_earned);
    }
        , 0);
                ...
                <div className="flex flex-col ml-4 flex-1/4">
                    <div className="flex items-center gap-2 mt-1">
                        ...
                        <div className="flex items-center gap-1">
                            <span className="bg-purple-500/20 text-purple-300 border-purple-500/30 text-sm font-semibold px-2 rounded border-1"> Level {level}</span>
                            {/* <span className="bg-blue-500/20 text-blue-300 border-blue-500/30 font-semibold text-sm px-2 rounded border-1"> {wins} Wins</span> */}
                            <span className=" text-[#e89454] border-yellow-500/30 font-bold px-2 animate-pulse"> {goldearned} Gold</span>
                    ...
                    <progress className="w-full h-2 bg-[#595757c8] rounded-full mt-2" value={progress * 10} max="100"></progress>
                </div>
