	...
	const intervalId = setInterval(() => {
			...
			if (Curentplayer.p1 == 1 && post == 0) {
				...
				async function postresult() {
					...
					const response = await fetch(
						"http://localhost:4000/games/:" + id + "/:" + oppid,
						{
							method: "POST",
							headers: {
							...
							body: JSON.stringify({
								...
								player1_gold_earned:
									Curentplayer.positions.win == 1 ? winnergold : losergold,
								player2_gold_earned:
									Curentplayer.positions.win == 1 ? losergold : winnergold,
		...
		if (Curentplayer.startgame && !Curentplayer.positions.win) {
			let { vx, vy } = CalculateballVelocity(
				Curentplayer.positions,
				Curentplayer.positions.angle
			);
			...
			if (gametype == "local") {
				...
				if (
					...
					10 + Curentplayer.positions.p2 + 2.5 <= 100
				)
					Curentplayer.positions.p2 = Curentplayer.positions.p2 + 2.5;
			}
			if (gametype == "localvsbot") {
			...
			if (
				Curentplayer.positions.ballx <= 100 &&
				Curentplayer.positions.ballx + vx >= 0
			) {
				if (Curentplayer.p1 == 0) {
					if (
						Curentplayer.positions.bally >= Curentplayer.positions.p2 - 10 &&
						Curentplayer.positions.bally <= Curentplayer.positions.p2 + 10 &&
						Curentplayer.positions.ballx + vx > 96
					) {
						let diff =
							(Curentplayer.positions.bally - Curentplayer.positions.p2) / 10;
						if (!diff) diff = 0.02;
						...
						Curentplayer.positions.bally >= Curentplayer.positions.p1 - 10 &&
						Curentplayer.positions.bally <= Curentplayer.positions.p1 + 10 &&
						Curentplayer.positions.ballx + vx < 4 &&
						Curentplayer.positions.direction == -1
					) {
						let diff =
							(Curentplayer.positions.bally - Curentplayer.positions.p1) / 10;
					...
					if (
						(Curentplayer.positions.bally + vy <= 2 ||
							Curentplayer.positions.bally + vy >= 98) &&
						!Curentplayer.positions.directionchanged
					) {
				...
				if (Curentplayer.p1 == 0) {
					Curentplayer.positions.direction == 1
						? Curentplayer.positions.score.p2++
						: Curentplayer.positions.score.p1++;
				}
				if (Curentplayer.oponent)
					Curentplayer.oponent.positions.score = Curentplayer.positions.score;
				if (
					(Curentplayer.positions.score.p2 > 11 &&
						Curentplayer.positions.score.p1 + 2 <=
							Curentplayer.positions.score.p2) ||
					(Curentplayer.positions.score.p1 > 11 &&
						Curentplayer.positions.score.p2 + 2 <=
							Curentplayer.positions.score.p1)
				) {
				...
				Curentplayer.positions.p1 = 50;
				Curentplayer.positions.p2 = 50;
				Curentplayer.positions.ballx = 50;
				Curentplayer.positions.bally = 50;
				Curentplayer.positions.angle = 0;
				Curentplayer.positions.speed = 1;
			}
			ws.send(JSON.stringify(Curentplayer.positions));
		}
	}, 20);
...
ws.on("connection", (ws, request) => {
	...
	ws.on("message", (msg) => {
		...
		if (data.type == "getpositions") {
			...
			playerid = data.id;
            if(playerid && oppid && playerid == oppid)
            {
